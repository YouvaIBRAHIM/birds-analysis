...
migration: migration ## Lance la migration de la base de données avec alembic
	docker exec -it my-backend alembic upgrade head

seed: compose ## Lance docker puis alimente la base de données
	docker exec -it my-backend python seed.py

stop: ## Stop les services du projet
	docker stop my-backend my-db
...
update: venv ## Mise à jour des packages
	docker cp ./back/requirements.txt my-backend:/app/
	docker exec my-backend pip install -r /app/requirements.txt
